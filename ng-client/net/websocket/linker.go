...
func (l *Linker) WriteMessage(data []byte) error {
	...
	return errors.New(fmt.Sprintf("%s w.conn is nil"))
}
...
func (l *Linker) checkTimeOut() {
	...
	log4plus.Info(fmt.Sprintf("%s start", funName))
	for {
		select {
		case <-l.heartbeatCh:
			l.heartbeat = time.Now()
		case <-l.exitCh:
			log4plus.Info(fmt.Sprintf("%s exitCh", funName))
			return
		}
	}
}
...
func NewLinker(mode NetMode, param string, handle uint64, remoteIp string, remotePort string, event OnEvent, conn *websocket.Conn) *Linker {
	...
	log4plus.Info(fmt.Sprintf("%s handle=[%d] remoteIp=[%s] remotePort=[%s]",
		funName, linker.handle, linker.remoteIp, linker.remotePort))
